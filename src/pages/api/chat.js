...
const handler = async (req, res) => {
  ...
  // Convert room_number to integer (without TypeScript syntax)
  const roomNumberInt = parseInt(room_number, 10);

  if (isNaN(roomNumberInt)) {
    return res.status(400).json({ message: "Room number tidak valid." });
  }
  ...
  if (req.method === "GET") {
    try {
      const query = `
        ...
        ORDER BY time ASC;
      `;
      const result = await client.query(query, [roomNumberInt]);
  ...
  // Handle POST request to send a new message
    ...
    if (!name || !email || !role || !message || isNaN(roomNumberInt)) {
    ...
    try {
      ...
      `;
      const values = [name, email, role, message, roomNumberInt];
